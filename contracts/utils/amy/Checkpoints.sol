...
library Checkpoints {
  ...
  function at(History storage self, uint256 pos)
    internal
    view
  ...
  function past(History storage self, uint256 index)
    internal
    view
    returns (uint256)
  {
    ...
    returns (uint256, uint256)
  {
    ...
    if (pos > 0 && self._checkpoints[pos - 1].index == block.number) {
      ...
      self._checkpoints.push(
        Checkpoint({
          index: SafeCast.toUint32(block.number),
          value: SafeCast.toUint224(value)
        })
      );
    }
    return (old, value);
