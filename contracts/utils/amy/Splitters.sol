...
library Splitters {
  ...
  function balanceOf(Splitter storage self, address account)
    internal
    view
    returns (uint256)
  {
  ...
  function mint(
    Splitter storage self,
    address account,
    uint256 amount
  ) internal {
    int256 virtualRelease = SafeCast.toInt256(
      _historicalRewardFraction(self, amount)
    );
  ...
  function toRelease(Splitter storage self, address account)
    internal
    view
    returns (uint256)
  {
        ...
        : SafeCast.toUint256(
          SafeCast.toInt256(
            FullMath.mulDiv(
              ...
              SafeCast.toUint256(
                SafeCast.toInt256(self._bounty) + self._released.total()
              )
            )
  ...
  function release(Splitter storage self, address account)
    internal
    returns (uint256)
  {
  ...
  function _totalHistoricalReward(Splitter storage self)
    private
    view
    returns (uint256)
  {
  ...
  function _historicalRewardFraction(Splitter storage self, uint256 amount)
    private
    view
    returns (uint256)
  {
    ...
    return
      amount > 0 && supply > 0
        ? FullMath.mulDiv(amount, supply, _totalHistoricalReward(self))
        : 0;
  }
}
