...
library AssetManager {
  struct Asset {
    function(Asset memory, address)
      internal
      view
      returns (uint256) _balanceHandler;
    function(Asset memory, address, uint256) internal _transferHandler;
    bytes _data;
  }

  function getBalance(Asset memory self, address account)
    internal
    view
    returns (uint256)
  {
  ...
  function _erc20Balance(Asset memory self, address account)
    private
    view
    returns (uint256)
  {
  ...
  /// ERC721
  // solhint-disable-next-line func-name-mixedcase
  function ERC721(IERC721 registry, uint256 tokenId)
    internal
    pure
  ...
  function _erc721Balance(Asset memory self, address account)
    private
    view
    returns (uint256)
  {
    (IERC721 registry, uint256 tokenId) = abi.decode(
      self._data,
      (IERC721, uint256)
    );
  ...
  /// ERC1155
  // solhint-disable-next-line func-name-mixedcase
  function ERC1155(IERC1155 registry, uint256 tokenId)
    internal
    pure
    returns (Asset memory result)
  {
  ...
  function _erc1155Balance(Asset memory self, address account)
    private
    view
    returns (uint256)
  {
    (IERC1155 registry, uint256 tokenId) = abi.decode(
      self._data,
      (IERC1155, uint256)
    );
